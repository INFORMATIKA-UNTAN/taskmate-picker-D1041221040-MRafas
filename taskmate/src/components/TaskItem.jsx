...
export default function TaskItem({ task, categories, onToggle, onDelete }) {
  function getDeadlineText(deadline) {
    ...
    // nolkan jam supaya hitungannya per hari
    ...
    if (diffDays < 0) {
      return { text: 'Overdue', style: { color: 'red' } };
    } else if (diffDays === 0) {
      return { text: 'Deadline hari ini', style: { color: '#16a34a' } };
    } else {
      return { text: `Sisa ${diffDays} hari`, style: { color: '#0f172a' } };
    }
  }
  ...
  return (
    <View style={[styles.card, isDone && styles.cardDone]}>
      ...
      <TouchableOpacity onPress={() => onToggle?.(task)} style={{ flex: 1 }}>
        ...
        {!!task.deadline && (() => {
          const info = getDeadlineText(task.deadline);
          return (
            <Text style={[styles.deadline, info?.style]}>
              {task.deadline} • {info?.text}
            </Text>
          );
        })()}
